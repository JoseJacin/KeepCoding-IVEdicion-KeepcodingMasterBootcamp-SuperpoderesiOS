...
class VolumeListViewController: UIViewController {
    ...
    // Es una propiedad que notifica a su coordinator 
...
extension VolumeListViewController: UICollectionViewDelegate {
	...
	func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        // Se le indica al ViewModel que retorne el Volume indicado por el index
		let volume = viewModel.item(at: indexPath.item)
        // Es un cloruse y se encarga de notificar a su coordinator que se ha seleccionado un Volume
